...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        clip_id: Union[torch.Tensor, float, int],
        encoder_hidden_states: torch.Tensor,
        class_labels: Optional[torch.Tensor] = None,
        attention_mask: Optional[torch.Tensor] = None,
        return_dict: bool = True,
        control = None,
        # stride: Union[torch.tensor, float, int] = None,
        ...
        if clip_id is not None:
            ...
            if not torch.is_tensor(clip_ids):
                # This would be a good case for the `match` statement (Python 3.10+)
                is_mps = sample.device.type == "mps"
                if isinstance(clip_id, float):
                    dtype = torch.float32 if is_mps else torch.float64
                else:
                    dtype = torch.int32 if is_mps else torch.int64
                clip_ids = torch.tensor([clip_ids], dtype=dtype, device=sample.device)
            elif len(clip_ids.shape) == 0:
                clip_ids = clip_ids[None].to(sample.device)
        ...
        for idx, downsample_block in enumerate(self.down_blocks):
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                sample, res_samples = downsample_block(
                    ...
                    lora_id = clip_ids,
                    feature_adapter = features_adapter[idx] if (self.adapter is not None and control is not None) else None
                )
            else:
        ...
        sample = self.mid_block(
            sample, emb, encoder_hidden_states=encoder_hidden_states, attention_mask=attention_mask,lora_id = clip_ids
        )
...
def conv_nd(dims, *args, **kwargs):
    ...
    if dims == 1:
        ...
        # return InflatedConv3d(*args,**kwargs)
        return nn.Conv2d(*args, **kwargs)
    elif dims == 3:
