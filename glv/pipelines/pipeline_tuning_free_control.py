...
from glv.util import get_noise_pred_single, init_prompt, next_step
...
@dataclass
class TuningFreeControlPipelineOutput(BaseOutput):
    videos: Union[torch.Tensor, np.ndarray]


class TuningFreeControlPipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, generator=None,stride=4,window_size=16,control=None):
        ...
        cond_embeddings = init_prompt("",self).chunk(2)[1]
        latents=init_latents.clone().detach()
    ...
    def __call__(
        ...
        **kwargs
        ...
        return self.gen_long(*args,**kwargs)
    ...
    def gen_long(
        ...
        **kwargs,
        ...
        # we use the clip-ddim to replace the simply noise adding.
        latents = self.prepare_latents(
            video, num_inference_steps, batch_size, num_videos_per_prompt, text_embeddings.dtype, device, generator,stride=stride,window_size=window_size,control=control
        )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for t_start, t_end in views:
                    ...
                    noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=text_embeddings,control=control_tmp).sample.to(dtype=latents_dtype)
                    # perform guidance
                    if do_classifier_free_guidance:
        ...
        return TuningFreeControlPipelineOutput(videos=video)
