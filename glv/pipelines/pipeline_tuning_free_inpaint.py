...
from diffusers.utils import deprecate, is_accelerate_available, is_accelerate_version, logging, randn_tensor, PIL_INTERPOLATION
from diffusers.pipeline_utils import DiffusionPipeline
from diffusers.pipelines.stable_diffusion import StableDiffusionPipelineOutput, BaseOutput
...
class TuningFreePipelineInpaint(DiffusionPipeline, TextualInversionLoaderMixin):
    r"""
    ...
    def prepare_mask_latents(
        ...
        if isinstance(generator, list):
            ...
            masked_image_latents = [
                self.vae.encode(masked_image[i : i + 1]).latent_dist.sample(generator=generator)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        bz = batch_size*2 if do_classifier_free_guidance else batch_size 
        mask = rearrange(mask, "(b f) c h w -> b c f h w", b=bz)
        masked_image_latents = rearrange(masked_image_latents, "(b f) c h w -> b c f h w", b=bz)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=prompt_embeds,control=control).sample
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for t_start, t_end in views:
                    ...
                    # concat latents, mask, masked_image_latents in the channel dimension
                    latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)
                    latent_model_input = torch.cat([latent_model_input, mask[:,:,t_start:t_end], masked_image_latents[:,:,t_start:t_end]], dim=1)
                    # predict the noise residual
                    noise_pred = self.unet(latent_model_input, t, encoder_hidden_states=prompt_embeds, control=control_tmp).sample
